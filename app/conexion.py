...
from simplejson import loads #Mejora el resultado de los JSON
...
"""Permite realizar las transacciones a SESAME"""
class connection:
...
if __name__=='__main__':
    ...
    #c.addnamespace('dbpedia','http://dbpedia.org/resource/')
    #c.addnamespace('wo', 'http://purl.org/ontology/wo/')
    #c.addnamespace('dc','http://purl.org/dc/elements/1.1/')
    #print c.get_in_context()
    
    #res=c.query('SELECT ?sub WHERE {?sub ?pred  ?obj . FILTER (regex(?obj, "Vivero")).}')
    #valores = []
    #val = []
    ...
    #for r in res:
    #    objetos = c.query('SELECT ?obj WHERE {<'+ r['sub']['value'] +'> ?pred  ?obj.}')
    #    valores.append(objetos)

    #for r in valores:
    #    print r[2]
    #    
    uno = "Todas las habitaciones inclueyn desayuno"
    dos = "Agroindustria Oleaginosa Corazon del Valle"
    print len(dos)
